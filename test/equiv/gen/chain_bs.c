/* chain_bs.c — prototype/chain.v 的 bit-sliced 版本。
 * 來源：prototype/cbench.c（yosys + prototype/gen.py 產生，830 cell），
 * 這裡只是把 benchmark 用的 main() 拿掉、把 eval_cycle 改成外部符號。
 * 要重新產生請跑 gen/regen.sh（需要 yosys）。      —— 產生物，勿手改 */
#include "chain_bs.h"

struct chain_state_ {   /* 所有 DFF 的狀態，每個 bit 一個 word */
    vec_t q18;
    vec_t q19;
    vec_t q20;
    vec_t q21;
